...
import { CheckCircle2, XCircle, ArrowUpRight, HelpCircle, Eye } from "lucide-react";
...
export default function PYQQuestionCard({ question, practiceIndex }) {
  ...
  const [checkedState, setCheckedState] = useState(null); // { isCorrect: boolean, message: string } | null
  ...
  const handleToggleMulti = (key) => {
    if (checkedState) return; // locked after check
  ...
  // Format origin label from original_paper_id or exam metadata
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-800 overflow-hidden transition-all hover:shadow-xl">
      ...
      <div className="p-5 md:p-7 space-y-6">
        ...
        {question.question_diagram_urls && question.question_diagram_urls.length > 0 && (
          <div className="flex flex-wrap gap-4 pt-2">
            {question.question_diagram_urls.map((imgUrl, i) => (
              <div key={i} className="rounded-xl border border-gray-200 dark:border-gray-700 p-2 bg-white dark:bg-gray-800 shadow-sm max-w-md">
                <img
                  src={imgUrl}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 text-sm pt-0.5 leading-relaxed text-gray-900 dark:text-gray-100">
                    ...
                    {opt.diagram_url && (
                      <img src={opt.diagram_url} alt={`Option ${key}`} className="max-h-24 object-contain mt-2 rounded" />
                    )}
                  </div>
                </button>
              );
        ...
        <div className="pt-4 border-t border-gray-100 dark:border-gray-800 flex flex-wrap items-center justify-between gap-3">
          ...
          {/* View Full Solution Button (opens /questions/[slug] in new tab) */}
          <Link
